...
module Day3 =
    ...
    module Part1 = 
        ...
        let calculateVisitedHouses(path: string) = 
            let mutable house = (0, 0)

            (Array.fold 
                (fun (houses: Set<int * int>) char -> 
                    house <- moveDirection house char
                    houses.Add house) 
                (set [(0,0)]) 
                (path.ToCharArray())
            ).Count
    ...
    module Part2 = 
        ...
        let calculateVisitedHousesWithRobo(path: string) =
            let mutable houses = set [(0,0)]
            let mutable santaHouse = (0, 0)
            let mutable roboHouse  = (0, 0)
            let mutable index = 0

            while index <= path.Length - 2 do
                santaHouse <- moveDirection santaHouse path[index]
                roboHouse  <- moveDirection roboHouse  path[index + 1]
                houses <- (houses.Add santaHouse).Add roboHouse
                index <- index + 2
            houses.Count