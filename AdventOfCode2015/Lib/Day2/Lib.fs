...
module Day2 =
    ...
    module Part1 =
        ...
        let CalculateWrappingSquareFeet(dimensionsList: string array) =
            Array.fold (fun total (dimensions: string) ->                 
                let dims = Array.map (int) (dimensions.Split('x'))
                let l = dims[0]
                let w = dims[1]
                let h = dims[2]
                let multDims = [|l*w; w*h; h*l|]
                total + ((2*l*w) + (2*w*h) + (2*h*l) + (Array.min multDims))
             ) 0 dimensionsList
    ...
    module Part2 =
        ...
        let CalculateRibbonFeet (dimensionsList: string array) =
            Array.fold (fun total (dimensions: string) ->                 
                let dims = Array.sort (Array.map (int) (dimensions.Split('x')))
                total + ((dims[0] + dims[1]) * 2) + (Array.fold (*) 1 dims) 
             ) 0 dimensionsList